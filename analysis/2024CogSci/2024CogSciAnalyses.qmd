...
write_figures <- here("analysis/2024CogSci/figures/")
...
subj_data <- read_csv(here("analysis/2024CogSci/subj_data_clean_2024-01-29.csv")) |> 
...
fuss_participants <- c("BI100", "BI123", "BI127")
length(fuss_participants)

calibration_error <- c("BI104REAL", "BI107", "BI112", "BI116", "BI1242")
...
summarize_across_subj$alpha_factor = factor()
...
first_look_plot <- first_look_by_participant |> 
  ...
  #geom_errorbar(aes(ymin = mean_duration-sd_start_time, ymax = mean_duration+sd_start_time)) +
  geom_point(data = first_look, aes(y = start_time/1000), alpha = .3) + 
  theme_minimal() + 
    scale_fill_manual(values = c("Reliable"="#3a86ff",
                                "Unreliable"="#ff006e")) +
  labs(x = "Sampled Speaker", 
       y = "Time of First Fixation (s)",
...
(first_look_lines / sample_duration_bar_lines) + plot_annotation(tag_levels = 'A')
ggsave(paste0(write_figures, "sampling_results_lines_stacked.pdf"), width = 3.5, height = 6) 
...
keep_training_trials %>%
  ...
  group_by(speakerType)  |>  
    dplyr::summarize(Mean = mean(n, na.rm=TRUE),
                     SD = sd(n, na.rm=TRUE)) |> 
  mutate(CI_low = Mean - (SD/sqrt(13))*1.96,
         CI_high = Mean + (SD/sqrt(13))*1.96)


print("Attention to proportions during training")
  
reliab_summarize_across_subj |> 
  mutate(CI_low = accuracy - (sd_accuracy/sqrt(13))*1.96,
         CI_high = accuracy + (sd_accuracy/sqrt(13))*1.96)

print("Proportion Looking test")

sample_duration_across_subj  |> 
  mutate(CI_low = mean_duration - (sd_duration/sqrt(13))*1.96,
         CI_high = mean_duration + (sd_duration/sqrt(13))*1.96)
...
first_look |> 
  ...
  summarize(mean_start_time = mean(start_time),
            ...
            mean_duration = mean(duration),
         sd_duration = sd(duration)) |> 
  mutate(start_CI_low = mean_start_time - (sd_start_time/sqrt(13))*1.96,
         start_CI_high = mean_start_time + (sd_start_time/sqrt(13))*1.96,
    duration_CI_low = mean_duration - (sd_duration/sqrt(13))*1.96,
         duration_CI_high = mean_duration + (sd_duration/sqrt(13))*1.96)
```
