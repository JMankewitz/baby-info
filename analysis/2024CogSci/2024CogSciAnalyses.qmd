...
remaining_participants <- subj_data |> filter(include == "yes" | include == "yes") |> pull(subjCode)
...
library(lme4)
library(lmerTest)
sample_duration_by_subj$condAOI_centered <- ifelse(sample_duration_by_subj$condAOI_factor == "Reliable", .5, -.5)
sample_duration_by_subj$age_mean_centered <- sample_duration_by_subj$age - mean(sample_duration_by_subj$age, na.rm = T)

model <- lmer(total_time_ms ~ condAOI_centered * age_mean_centered + (1 | subjCode), data = sample_duration_by_subj)
summary(model)
```
